...
class HospitalService {
      ...
      fs.createReadStream(csvFilePath)
        ...
        .on('end', async () => {
          try {
            ...
            // DB에 bulk insert
            await this.Hospital.bulkCreate(results, {
  ...
  async findNearbyHospitals(latitude, longitude, radiusKm = 5) {
    try {
      ...
      const allHospitals = await this.Hospital.findAll({
  ...
  async searchByKeyword(keyword, limit = 20) {
    try {
      const hospitals = await this.Hospital.findAll({
        where: {
          [db.Sequelize.Op.or]: [
  ...
  async getHospitalDetail(hospitalId) {
    try {
      const hospital = await this.Hospital.findByPk(hospitalId);
